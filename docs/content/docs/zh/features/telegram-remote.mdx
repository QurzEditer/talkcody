...
```
您: /newbot
BotFather: 好的，请为新机器人选择一个名称。
您: TalkCody 远程控制
BotFather: 好。现在请为机器人选择一个用户名。它必须以 bot 结尾，例如：TetrisBot 或 tetris_bot。
您: talkcody_remote_bot
BotFather: 大功告成！...
...
方法一：通过日志查看
1. 先在 TalkCody 中配置好 Bot Token 并启用远程控制
2. 在 Telegram 中给 Bot 发送一条消息（任意内容）
3. 在 TalkCody 日志中查看 `chat_id`

方法二：使用工具 Bot
1. 在 Telegram 中搜索 **@userinfobot**
2. 发送任意消息
3. 机器人会回复你的用户 ID（即 Chat ID）

<Callout type="info">
Chat ID 是一个数字，私聊通常为正值（如 `123456789`），群聊通常为负值（如 `-1001234567890`）。当前版本仅支持私聊。
</Callout>

### 3. 确保桌面端 TalkCody 正在运行
