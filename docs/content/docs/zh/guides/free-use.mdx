...
| **OpenCode Zen** | 免费模型 | 专为编码优化，提供免费 GLM-4.7 | 需要注册账户 |
...
OpenCode Zen 提供免费的 GLM-4.7 模型，这是一个专为编码任务优化的高性能大模型，无需支付额外费用即可在 TalkCody 中使用。

### 配置方法

详细的 API Key 配置步骤请参考 [OpenCode Zen API Key 配置指南](../configuration/api-keys#opencode-zen)。

配置完成后，在 TalkCody 的 **模型设置** 中选择 GLM-4.7 模型即可免费使用。
